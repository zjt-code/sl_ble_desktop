...
uint32_t Peripheral::get_characteristic_prop(BluetoothUUID service, BluetoothUUID characteristic)
{
    if (!is_connected()) {
        throw Exception::OperationFailed();
    }

    return internal_->get_characteristic_prop( service,  characteristic);
}